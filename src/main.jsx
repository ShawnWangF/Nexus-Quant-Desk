...
function MarketMapView({ watchlist = [], liveQuotes = {}, onSelect }) {
  ...
  const scopedRows = rows.filter((row) => scope === '全部自选' ? watchlist.includes(row[0]) : scope === '涨跌幅' ? true : watchlist.includes(row[0]))
    .map((row) => { const quote = liveQuotes[row[0]]; return quote ? [row[0], quote.name || row[1], quote.price.toFixed(2), `${quote.change >= 0 ? '+' : ''}${quote.change.toFixed(2)}%`, quote.volume ? Number(quote.volume).toLocaleString() : row[4], row[5]] : row })
    .sort((a, b) => scope === '涨跌幅' ? Math.abs(parseFloat(b[3])) - Math.abs(parseFloat(a[3])) : 0)
  return <section className="view-panel"><div className="view-toolbar"><div><p className="eyebrow">LIVE MARKET MAP</p><h2>市场地图</h2><span className="view-note">筛选自选行情并点击标的切换主图。</span></div><div className="segmented-control">{['持仓相关', '全部自选', '涨跌幅'].map((item) => <button className={scope === item ? 'selected' : ''} key={item} onClick={() => setScope(item)}>{item}</button>)}</div></div><div className="market-table"><div className="market-table-head"><span>标的</span><span>最新价</span><span>涨跌幅</span><span>成交量</span><span>状态</span><span>操作</span></div>{scopedRows.map((row) => <div className="market-table-row" key={row[0]}><div><strong>{row[0]}</strong><small>{row[1]}</small></div><strong>{row[2]}</strong><span className={row[3].startsWith('+') ? 'positive' : 'negative'}>{row[3]}</span><span className="mono-muted">{row[4]}</span><span className={`market-state ${row[5] === '承压' ? 'danger' : row[5] === '高波动' ? 'warn' : ''}`}>{row[5]}</span><button className="small-action" onClick={() => onSelect?.(row[0])}>查看图表</button></div>)}</div>{!scopedRows.length && <div className="empty-state"><BarChart3 size={24} /><strong>当前筛选暂无标的</strong><span>先在市场现状中加入自选。</span></div>}</section>
}
...
function MarketStatusPanel({ watchlist, selectedSymbol, liveQuotes, onSelect, onAdd, onRemove }) {
  ...
  return <section className="panel market-status-panel">
    ...
    <div className="watchlist-grid">{watchlist.map((symbol) => { const quote = liveQuotes[symbol]; const name = quote?.name || candidateStocks.find((item) => item.symbol === symbol)?.name || symbol; return <button className={`watch-card ${symbol === selectedSymbol ? 'selected' : ''}`} key={symbol} onClick={() => onSelect(symbol)}><span><strong>{name}</strong><small>{symbol}</small></span><b>{quote?.price ? formatNumber(quote.price, quote.price < 10 ? 3 : 2) : '--'}</b><em className={quote?.change >= 0 ? 'positive' : 'negative'}>{quote ? `${quote.change >= 0 ? '+' : ''}${quote.change.toFixed(2)}%` : '等待行情'}</em><i onClick={(event) => { event.stopPropagation(); onRemove(symbol) }} aria-label={`移除 ${symbol}`}>×</i></button> })}</div>
    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        {activeNav === '总览' ? <div className="dashboard-grid">
          ...
          <MarketStatusPanel watchlist={watchlist} selectedSymbol={watchSymbol} liveQuotes={liveQuotes} onSelect={setWatchSymbol} onAdd={addWatch} onRemove={removeWatch} />
        ...
        </div> : activeNav === '市场地图' ? <MarketMapView watchlist={watchlist} liveQuotes={liveQuotes} onSelect={setWatchSymbol} /> : activeNav === '持仓与风险' ? <PositionsView positions={accountPositions} liveQuotes={liveQuotes} onTrade={openTradeTicket} tradeLog={account.trades} onReset={resetAccount} /> : activeNav === '策略实验室' ? <StrategyView /> : <AlertsView signals={signals} setSignals={setSignals} setWatchSymbol={setWatchSymbol} />}
